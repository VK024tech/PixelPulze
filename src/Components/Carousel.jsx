...
function Carousel() {
  ...
  return (
    ...
    >
      <div className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-pale-white dark:from-black to-transparent z-10 w-28 sm:w-xs md:w-md   pointer-events-none" />
      <div className="absolute top-0 bottom-0 right-0 bg-gradient-to-l from-pale-white dark:from-black to-transparent z-10 w-28 sm:w-xs md:w-md pointer-events-none" />
      <motion.div className="flex shrink-0 justify-start gap-6 " ref={scope}>
        {images.map((curr, index) => {
          return (
            <img
              key={curr + index}
              className="w-3xs md:w-sm rounded-2xl "
              src={curr}
              alt=""
            />
          );
